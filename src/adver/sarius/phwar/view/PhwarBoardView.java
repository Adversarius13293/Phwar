...
public class PhwarBoardView extends GridPane implements ModelListener {
	...
	// I dont like it, but since the controller is part of the view, I cant keep
			...
			if (node instanceof StackPane) {
				...
				for (ListIterator<Node> iter = hexaPane.getChildren().listIterator(); iter.hasNext();) {
					...
					if (n instanceof ParticleStackPane) {
						...
						Particle particle = PhwarBoard.getParticle(hexa.getPosX(), hexa.getPosY(), board.getParticles())
								.orElse(null);
						if (particle != null) {
	...
	private void constructView() {
		...
		for (int col = 0; col < boardSize * 2 + 1; col++) {
			// only use every second row, alternating depending on the column. Also
			// alternating start position depending on board size.
			for (int row = (col + boardSize % 2) % 2; row < boardSize * 4 + 1; row += 2) {
				...
				if (!board.isInsideBoard(boardX, boardY)) {
					// should already skip them while iterating. But this way I keep the logic for
